...
from hunter.core.Basic import Basic
from hunter.utils.log import LogCaptor
from hunter.utils.expandjsonencode import ExpandJsonEncoder
from hunter.internet.proxys import asy_rand_choi_pool
from hunter.core.http.make_args import MakeArgs
from hunter.core.http._requests import AsyncRequest
from hunter.middlewares.middleware_loader import load_middlewares
from hunter.middlewares.middleware_manager import MiddlewareManager
from hunter.core.http.request_objects import Response, Requests, FormRequests, PatchRequests
from hunter.core.data_class.data_class import RequestParams
from hunter.utils.single_tool import is_json, per_dic_plus
from hunter.items.baseitem import Item
from hunter.piplines.pipeline_loader import load_pipelines
from hunter.piplines.pipelines_manager import PipelineManager
from hunter.core.instancemeta import Base


class ManagerMemory(Basic, Base):
    ...
    def __init__(self):
        ...
        self.__retry_http_codes = self._settings.RETEY_HTTP_CODES
    ...
    async def process_message(self, task: Any):
        """消息处理函数"""
        self.last_time = time.time()
        self.__Concurrency -= 1
        while self.__Concurrency < 0:
            self.logger.debug(f'The request queue is full. Please wait for the results to be returned.')
            time.sleep(1)
        flag = is_json(task)
        if not flag:  # 判断是否为请求消息，如果不是的话
            if inspect.isgeneratorfunction(self.parse_only):
                message = self.parse_only(task=task).__next__()  # 获取生成器元素
                callback = message.callback.__name__  # 获取callback函数名
                self.callback_map[callback] = message.__dict__.get('level') or 0  # 定义callback优先级
                message.level = self.callback_map[callback] + 1  # 增加消息优先级
                # self.async_thread_pool.submit(self.push, item=message)  # 多线程数据处理
                asyncio.create_task(self.push(item=message))
            else:
                # self.async_thread_pool.submit(self.parse_only, task=task)  # 多线程数据处理
                asyncio.create_task(self.parse_only(task=task))
        ...
        if flag:  # 判断是否为请求消息，如果是的话
            ...
            # requests_task = asyncio.run_coroutine_threadsafe(
            #     self.make_requests(RequestParams(**params)), self.new_loop
            # )
            ...
            while request_params.retry_count < self.__max_request:
                ...
                try:
                    ...
                    if (not request_params.is_proxy) and (self.__max_request):
                        ...
                        await self.push(item=json.dumps(mess, cls=ExpandJsonEncoder))
                        self.logger.error(f'{repr(e)} Returning to the queue {str(request_params.task)}', exc_info=True)
                        break
            else:
                response_last = Response(
    ...
    # @count_time
    async def __deal_fun(self, callback: Callable, response_last: dict):
        """回调函数处理"""
        try:
            ...
            if inspect.isasyncgenfunction(getattr(self, callback)):
                async for item in getattr(self, callback)(response=response_last):
                    if isinstance(item, (Requests, FormRequests, PatchRequests)):
                        ...
                        # print(f'{callback}优先级：{item.level}')
                        await self.push(item=item)
