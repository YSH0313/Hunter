...
from hunter.commands.config import white_text, blue_text, purple_text, reset_all
from hunter.core import ManagerMemory
from hunter.core.http.request_objects import Requests, Response
from hunter.middlewares.middleware_manager import MiddlewareManager
from hunter.utils.single_tool import now_time


class MemorySpider(ManagerMemory):
    ...
    async def execute(self):
        ...
        # 开启消费者
        # await asyncio.create_task(self.consumer_memory())
        consumer_task = asyncio.create_task(self.consumer_memory())
    ...
    async def shutdown_spider(self, spider_name: str):
        ...
        try:
            while True:
                ...
                self.logger.debug(f'{white_text}{round(now_time - self.last_time)} {blue_text}seconds have passed since the last dequeue operation, and there is {purple_text}{len(self._queue)} {blue_text}message remaining in the queue.{reset_all}')

                if ((now_time - self.last_time >= self.__Waiting_time) and (len(self._queue) == 0)):
                    # do something
                    break
